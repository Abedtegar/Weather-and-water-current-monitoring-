...
void rgbLedTestInit() {
  ...
  gRgb.clear();
  gRgb.show();
  // Default to disconnected until WiFi reports otherwise.
  setRgbColor(255, 0, 0);
}

void rgbLedTestUpdate(uint32_t nowMs) {
  if (nowMs - gLastRgbUpdateMs < kRgbUpdateIntervalMs) {
    return;
  }

  updateWiFiIndicator();
  gLastRgbUpdateMs = nowMs;
}
